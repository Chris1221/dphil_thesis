...
\begin{table}
    ...
    \slcaption{\label{tab:table:mll_edger_genes}Enriched genic regions along with their corrected FDR P value (Q-value) from the top 500 regions differentially accessible betwen MLL-AF4 cells and BCP using edgeR.}
\end{table}
...
Individual regions such as the putative enhancer cluster described in \Cref{fig:mll_enhancer_cluster} show a simimlar pattern, with some accessibility in early progenitor cells which is mostly lost by terminal differentiation (\Cref{fig:lymph_enhancer_cluster}). 

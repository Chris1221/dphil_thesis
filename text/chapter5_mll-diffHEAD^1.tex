...
%DIF <  General statement of intent.
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Substantial advances in the treatment and supportive care of children with }\gls{all} \DIFadd{have resulted in over 90\% of patients surviving beyond five years of diagnosis \mbox{%DIFAUXCMD
\cite{Inaba2021}}\hspace{0pt}%DIFAUXCMD
. However, one particular subset of high-risk paediatric }\glspl{all} \DIFadd{have remained mostly resistant to current treatment regimes, resulting in the second major cause of cancer deaths among paediatric patients being refractory and relapsed }\gls{all} \DIFadd{\mbox{%DIFAUXCMD
\cite{Inaba2021}}\hspace{0pt}%DIFAUXCMD
. Chromosomal translocations of the }\DIFaddend mll \DIFdelbegin \DIFdel{gene, previously known as MLL }\DIFdelend \DIFaddbegin \DIFadd{(also known as the Mixed Lineage Leukemia gene (MLL) }\DIFaddend or MLL1\DIFdelbegin \DIFdel{, is a crucial epigenetic regulator required for definitive hematopoiesis \mbox{%DIFAUXCMD
\cite{Ernst2004a}}\hspace{0pt}%DIFAUXCMD
. It is also frequently involved in chromosomal translocations causing a treatment resistant leukemia with especially poor prognosis \mbox{%DIFAUXCMD
\cite{Krivtsov2007}}\hspace{0pt}%DIFAUXCMD
. Though outcomes for acute leukemias in children has improved vastly, the five year event-free survival rate for infant leukemias remains less than 50\% despite extensive treatment efforts and supportive care \mbox{%DIFAUXCMD
\cite{Rice2020}}\hspace{0pt}%DIFAUXCMD
. Recent work has demonstrated the important roles of chromatin remodelling in KMT2A-recombinant (}\DIFdelend \DIFaddbegin \DIFadd{) gene are a major cause of incurable paediatric and infant leukemias, and represent between 5-10\% of }\gls{all} \DIFadd{cases in all age groups \mbox{%DIFAUXCMD
\cite{Rice2020b}}\hspace{0pt}%DIFAUXCMD
. The }\gls{fp} \DIFadd{changes the transcriptome sufficiently to motivate leukemogenesis, however the altered regulatory programs involved are only just beginning to be elucidated. This disease represents one of the rare instances where a single genetic change is sufficient to cause cancer, in essence a single oncoprotein directing the performance of all ten canonical hallmarks of cancer \mbox{%DIFAUXCMD
\cite{Rao2015,Hanahan2011}}\hspace{0pt}%DIFAUXCMD
. An understanding of this minimal set of carcinogenic regulatory pathways is therefore of generalisable interest, and a study of this system has already yielded actionable insights into other, more prevalent, diseases. Examples include BET, LSD1, and Menin inhibitors, each originally shown to be effective in cellular and xenographed murine models of }\DIFaddend KMT2Ar \DIFdelbegin \DIFdel{) leukemiagenesis, though the specifics of the differentially accessible regulatory programs active in infant KMT2Ar remains unknown \mbox{%DIFAUXCMD
\cite{Erfurth2003}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{leukemia \mbox{%DIFAUXCMD
\cite{Issa2021,Fang2019,Shorstova2021,Abedin2016}}\hspace{0pt}%DIFAUXCMD
. The extend to which }\glspl{fp} \DIFadd{creates or reuses existing regulatory pathways is not well understood. One method to assess the complement of active regulatory elements defining a certain cell type is through NGS-based chromatin accessible assays such as }\gls{atac} \DIFadd{\mbox{%DIFAUXCMD
\cite{Minnoye2021, Liu2020}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . In this chapter, I \DIFdelbegin \DIFdel{propose the use of the previously developed topic modelling approach BLDA to identify accessible regulatory programs in }\DIFdelend \DIFaddbegin \DIFadd{adopt the machine learning approach developed in the previous chapter to study and subsequently character patterns in accessible regulatory elements within }\DIFaddend KMT2A-AFF1 \DIFdelbegin \DIFdel{leukemia }\DIFdelend patients.  

\DIFdelbegin \DIFdel{blah blah blah
}\DIFdelend %DIF >  alter the regulatory capacity of the cell type of origin in order to  is now well understood influences the expression of genes directly and indirectly; in this chapter, I propose the use of the previously developed topic modelling approach BLDA to identify and subsequently characterise accessible regulatory programs in KMT2A-AFF1 leukemia patients. 
\DIFaddbegin 


%DIF >  General statement of intent.
%DIF >  The \gls{mll} gene, previously known as MLL or MLL1, is a crucial epigenetic regulator required for definitive hematopoiesis \cite{Ernst2004a}. It is also frequently involved in chromosomal translocations causing a treatment resistant leukemia with especially poor prognosis \cite{Krivtsov2007}. 

%DIF >  Though outcomes for acute leukemias in children has improved vastly, the five year event-free survival rate for infant leukemias remains less than 50\% despite extensive treatment efforts and supportive care \cite{Rice2020}. Recent work has demonstrated the important roles of chromatin remodelling in KMT2A-recombinant (KMT2Ar) leukemiagenesis, though the specifics of the differentially accessible regulatory programs active in infant KMT2Ar remains unknown \cite{Erfurth2003}. In this chapter, I propose the use of the previously developed topic modelling approach BLDA to identify accessible regulatory programs in KMT2A-AFF1 leukemia patients.  
...
Similar to the yeast analog SET1, a C-terminal SET domain confers histone \DIFdelbegin \DIFdel{H3K4 }\DIFdelend \DIFaddbegin \DIFadd{3 lysine 4 }\DIFaddend methyltransferase activity while the N-terminal \DIFdelbegin \DIFdel{is responsible for binding with three AT-hook motifs and four PHD fingers \mbox{%DIFAUXCMD
\cite{JJ2003, Hsieh2003}}\hspace{0pt}%DIFAUXCMD
. While functional, }\DIFdelend \DIFaddbegin \DIFadd{contains a CXXC domain which binds to unmethylated CpG islands and accounts for much of the binding potential of both wildtype and fused }\DIFaddend KMT2A \DIFdelbegin \DIFdel{is responsible for region specific methylation deposition responsible for a diverse set of functions including marking enhancer elements and promoting transcription initiation \mbox{%DIFAUXCMD
\cite{Wang2009}}\hspace{0pt}%DIFAUXCMD
. 
In KMT2Ar }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{JJ2003, Hsieh2003, Cierpicki2010,Ayton2004,LE2013}}\hspace{0pt}%DIFAUXCMD
. 
%DIF >  While functional, KMT2A is responsible for region specific histone 3 lysine 4 methylation deposition responsible for a diverse set of functions including marking enhancer elements and promoting transcription initiation \cite{Wang2009}. 
In }\gls{fp} \DIFaddend driven leukemias, the cleaved N terminus instead associates with one of up to \DIFdelbegin \DIFdel{ninety }\DIFdelend \DIFaddbegin \DIFadd{one hundred and twenty }\DIFaddend C-terminal proteins, conferring a huge range of functional activities and \DIFdelbegin \DIFdel{interupting }\DIFdelend \DIFaddbegin \DIFadd{interrupting }\DIFaddend its normal function as a \DIFaddbegin \DIFadd{H3K4 }\DIFaddend methyltransferase \cite{Hsieh2003}. 
The fusion (KMT2A-FP) is able to recruit new \DIFdelbegin \DIFdel{cofactors }\DIFdelend \DIFaddbegin \DIFadd{co-factors }\DIFaddend such as DOT1L, whose \DIFdelbegin \DIFdel{abberant }\DIFdelend \DIFaddbegin \DIFadd{aberrant }\DIFaddend methylation profile at H3K79 sites is highly discriminatory for genes activated by the fusion protein and believed to be crucial for the transforming activity of KMT2A-FPs \cite{YOkada2005, KMBernt2011}. Incidentally, clinically targeting DOT1L is generally not sufficient to have any effect on many KMT2Ar cell lines, though some early evidence suggests that targetting DOT1L alongside other key cofactors like menin may show more success \cite{Dafflon2016}. This demonstrates the resiliency and redundancy in KMT2A-FP driven leukemias, and the need to understand the full complement of epigenomic changes involved in leukemia maintenance.  

\DIFdelbegin \DIFdel{Much like its analog COMPASS, KMT2A has an intimate relationship with chromatin remodelling factors such as INI1 and SNF5 \mbox{%DIFAUXCMD
\cite{Cenik2021, Sugeedha2021}}\hspace{0pt}%DIFAUXCMD
. This family of epigenetic modifiers termed SWI/SNF (historically called SWItch/Sucrose non-fermentable due to its involvement in yeast sucrose fermentation pathways) was first characterized in yeast, and is involved in ATP-dependant nucleosome mobilization during transcription and the cell cycle \mbox{%DIFAUXCMD
\cite{C2015}}\hspace{0pt}%DIFAUXCMD
. In humans, mutations within SWI/SNF chromatinremodelling complexes are incredibly common in cancer; as an example, 57\% of ovarian clear cell carcinomas contained inactivating mutations along the length of a BAF complex subunit ARID1A (ref 45). Similarly, inactivation of INI1 causes a rare and extremely aggressive childhood cancer called malignant rhabdoid tumours (MRTs), nominating INI1 as a recessive tumor suppressor and raising key questions about its dysregulation in KMT2Ar cancers due to direct protein-protein interactions \mbox{%DIFAUXCMD
\cite{I1998}}\hspace{0pt}%DIFAUXCMD
. See \mbox{%DIFAUXCMD
\cite{C2015} }\hspace{0pt}%DIFAUXCMD
for a comprehensive review of the roles of the human SWI/SNF complex in malignancies. }\DIFdelend %DIF > Much like its analog COMPASS, KMT2A has an intimate relationship with chromatin remodelling factors such as INI1 and SNF5 \cite{Cenik2021, Sugeedha2021}. This family of epigenetic modifiers termed SWI/SNF (historically called SWItch/Sucrose non-fermentable due to its involvement in yeast sucrose fermentation pathways) was first characterized in yeast, and is involved in ATP-dependant nucleosome mobilization during transcription and the cell cycle \cite{C2015}. In humans, mutations within SWI/SNF chromatin remodelling complexes are incredibly common in cancer; as an example, 57\% of ovarian clear cell carcinomas contained inactivating mutations along the length of a BAF complex subunit ARID1A (ref 45). Similarly, inactivation of INI1 causes a rare and extremely aggressive childhood cancer called malignant rhabdoid tumours (MRTs), nominating INI1 as a recessive tumor suppressor and raising key questions about its dysregulation in KMT2Ar cancers due to direct protein-protein interactions \cite{I1998}. See \cite{C2015} for a comprehensive review of the roles of the human SWI/SNF complex in malignancies. 
...
\DIFdelbegin \DIFdel{Translocations involving KMT2A are a negative pronostic factor when the leukemia presents as }%DIFDELCMD < \gls{all}%%%
\DIFdel{, but not as }%DIFDELCMD < \gls{aml} %%%
\DIFdel{in infants \mbox{%DIFAUXCMD
\cite{Meyer2014}}\hspace{0pt}%DIFAUXCMD
. The former, however, accounts for upwards of 80\% of leukemias involving KMT2Ar \mbox{%DIFAUXCMD
\cite{Rice2020}}\hspace{0pt}%DIFAUXCMD
. The reasons for this difference in prognosis are not well understood, especially as cancers with this etiology are especially plastic with regards to their presentation. Leukemias initially presenting as }%DIFDELCMD < \gls{all} %%%
\DIFdel{often present as }%DIFDELCMD < \glspl{aml} %%%
\DIFdel{upon relapse (which is a frequent occurance among KNT2Ar leukemias).  however the underlying chromatin landscape of }\DIFdelend \DIFaddbegin \DIFadd{Recent research points to disturbances in the epigenetic regulation of gene expression as a potential mechanism for leukemogenesis \mbox{%DIFAUXCMD
\cite{Krivtsov2007}}\hspace{0pt}%DIFAUXCMD
. The amount of expressed gene product in a specific cellular system represents a complex interplay between DNA binding transcription factors occupying sequences at }\glspl{cre} \DIFadd{and a tight control over the chromatin structure at these sites \mbox{%DIFAUXCMD
\cite{Cierpicki2010}}\hspace{0pt}%DIFAUXCMD
. The fundamental unit of eukaryotic chromatin, }\DIFaddend the \DIFdelbegin \DIFdel{cancer cells in this case is able to produce gene expression programs resembling either of the two diseases, or a phenotypically intermediate form known as mixed lineage leukemia (MLL). Though KMT2A has well described direct interactions with many different proteins including cell cycle regulators like E2F and HCF, Cyp33, HDAC complexes, histone acetylases (CBP and P300) and the previously mentioned chromatin remodelling factors INI1 }\DIFdelend \DIFaddbegin \DIFadd{nucleosome, is regulated and enzymatically modified by a variety of protein complexes that both promote the recruitment of additional regulatory proteins and mobilize nucleosomes to allow polymerase direct access to the sequence \mbox{%DIFAUXCMD
\cite{Klemm2019a,Hu2016}}\hspace{0pt}%DIFAUXCMD
. In theory, the presence of both transcription factors and regulatory histone modifications can be assessed by }\gls{chip}\DIFadd{. however practical issues involving the efficacy of a particular antibody and the prohibitive cost involved in exhaustively sequencing each known mark make this impractical except in well-studied }\DIFaddend and \DIFdelbegin \DIFdel{SNF5, the degree to which the fusion proteins influence the transcriptome is only just beginning to be illuminated \mbox{%DIFAUXCMD
\cite{Sugeedha2021}}\hspace{0pt}%DIFAUXCMD
. Its affinity for binding to specific regions of the genome is not entirely determined by the actual sequence but also partially the structure of the DNA, with early results indicating binding affinity for cruciform DNAs with no clear sequence homology \mbox{%DIFAUXCMD
\cite{NJ1994}}\hspace{0pt}%DIFAUXCMD
. Much recent research points to disturbances in the epigenetic regulation of gene expression as a potential mechanism for leukemiagenesis \mbox{%DIFAUXCMD
\cite{Krivtsov2007}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{well-defined model cell systems. A well-known proxy for the activity of a region is its accessibility, which can be efficiently assayed with ATAC or DNAse sequencing without the complication of selecting efficient antibodies or the need to conduct a separate control experiment \mbox{%DIFAUXCMD
\cite{Song2011,Minnoye2021,Thurman2012,Boyle2008}}\hspace{0pt}%DIFAUXCMD
. This celltype-specific complement of accessible chromatin is highly reflective of the unique regulatory landscape of each individual cell \mbox{%DIFAUXCMD
\cite{Thurman2012,Zhang2021,Schulz2019}}\hspace{0pt}%DIFAUXCMD
. We use the previously developed topic modelling approach, }\gls{blda}\DIFadd{, to identify differentially accessible regions across a population of leukemia patients and cell lines compared to developmentally normal lymphoid cells}\DIFaddend . 

In this chapter, I concentrate on the most frequent fusion partner of KMT2A, AFF1 \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{(previously known as }\DIFaddend AF4\DIFdelbegin \DIFdel{as it was previously known \mbox{%DIFAUXCMD
\cite{Meyer2017}}\hspace{0pt}%DIFAUXCMD
. Many fusion partners of }\DIFdelend \DIFaddbegin \DIFadd{) \mbox{%DIFAUXCMD
\cite{Meyer2017}}\hspace{0pt}%DIFAUXCMD
. AFF1 belongs to a higher-order protein complex implicated in transcriptional regulation and involving many of the most frequently fused }\DIFaddend KMT2A \DIFdelbegin \DIFdel{belong to a network of transcriptional regulators involved in chromatin remodeling, including AF4, and the disruption of this network may be important for leuekmiagenesis \mbox{%DIFAUXCMD
\cite{Erfurth2003}}\hspace{0pt}%DIFAUXCMD
. 
In fact, recent }\DIFdelend \DIFaddbegin \DIFadd{partners \mbox{%DIFAUXCMD
\cite{Yokoyama2010}}\hspace{0pt}%DIFAUXCMD
. 
Recent }\DIFaddend evidence has suggested that the KMT2A-AFF1 fusion protein is involved in a complex feed-forward gene regulatory network (GRN) involving \DIFdelbegin \DIFdel{many well-known proteins }\DIFdelend \DIFaddbegin \DIFadd{transcription factors }\DIFaddend such as RUNX1 \DIFdelbegin \DIFdel{and CASP9 }\DIFdelend as well as some which have no previously-described function \cite{Harman2021, Wilkinson2013}. 
Outside of \DIFdelbegin \DIFdel{well-characterised }\DIFdelend \DIFaddbegin \DIFadd{well-characterized }\DIFaddend direct binding events where KMT2A-AFF1 is able to regulate the expression of key genes such as proto-oncogene MyC and BCL2, \DIFdelbegin \DIFdel{its regulation of and interactions with downstream gene targets is almost completely unknown. This is especially true in the case of infant leukemias with dismal prognosis, where a unique fetal-specific gene expression profile may contribute to the otherwise unexplained age-related differences in prognosis \mbox{%DIFAUXCMD
\cite{Rice2020a}}\hspace{0pt}%DIFAUXCMD
. Fetal lymphopoiesis remains poorly defined, however clearly defining the expression and accessibility profiles as they differ from adult progenitor cells is key to understanding leukemiagenesis}\DIFdelend \DIFaddbegin \DIFadd{the direct and indirect regulation of transcription by the fusion protein remains an active area of research. 
An investigation into the patterns of accessible chromatin in leukemic samples would provide insight into the differential usage of enhancer and promoter elements as well as the pathways which may drive their use}\DIFaddend . 
Recently, \DIFdelbegin \DIFdel{two }\DIFdelend \DIFaddbegin \DIFadd{a well-characterized set of }\DIFaddend CD19$^+$ \DIFdelbegin \DIFdel{B-cell progenitors were identified in fetal populations, }\DIFdelend \DIFaddbegin \DIFadd{lymphoid progenitors was identified in infants \mbox{%DIFAUXCMD
\cite{OByrne2019}}\hspace{0pt}%DIFAUXCMD
. 
These include }\DIFaddend a CD10$^+$ ProB (PB) cell type resembling adult progenitors and a completely novel Pre-ProB (PPB) CD10$^-$ celltype almost undetectable in adult bone marrow \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{OByrne2019}}\hspace{0pt}%DIFAUXCMD
. Additionally, their transcriptomic profile is dissimilar from the small numbers of PPB cells able to isolated in adults, and share expression signatures with CD10$^-$ infant }\DIFdelend \DIFaddbegin \DIFadd{and with a gene expression profile resembling }\DIFaddend \gls{all} \DIFdelbegin \DIFdel{blast cells }\DIFdelend \DIFaddbegin \DIFadd{blasts }\DIFaddend \cite{OByrne2019}. 
These cells \DIFdelbegin \DIFdel{therefore }\DIFdelend represent the earliest identified \DIFdelbegin \DIFdel{fetal-restricted committed lymphoid progenitor}\DIFdelend \DIFaddbegin \DIFadd{committed lymphoid progenitors}\DIFaddend , suggesting that they may provide a permissive environment for leukemia origination. 
\DIFaddbegin \DIFadd{As such, they represent a reasonable system to treat as a developmentally "normal" lymphoid cell against which we compare leukemic samples.
}\DIFaddend 

%DIF >  Fetal lymphopoiesis remains poorly defined, however clearly defining the expression and accessibility profiles as they differ from adult progenitor cells is key to understanding leukemiagenesis. Recently, two CD19$^+$ B-cell progenitors were identified in fetal populations, a CD10$^+$ ProB (PB) cell type resembling adult progenitors and a completely novel Pre-ProB (PPB) CD10$^-$ celltype almost undetectable in adult bone marrow \cite{OByrne2019}. Additionally, their transcriptomic profile is dissimilar from the small numbers of PPB cells able to isolated in adults, and share expression signatures with CD10$^-$ infant \gls{all} blast cells \cite{OByrne2019}. These cells therefore represent the earliest identified fetal-restricted committed lymphoid progenitor, suggesting that they may provide a permissive environment for leukemia origination.  
\DIFaddbegin 

\DIFaddend This chapter aims to use topic modelling to describe regulatory programs within four \DIFdelbegin \DIFdel{childhood }\DIFdelend \DIFaddbegin \DIFadd{KMT2A }\DIFaddend leukemia patients, contrasting them against \DIFaddbegin \DIFadd{these }\DIFaddend two closely related healthy fetal cell types, PB, and PPB. We also compare these samples against two cellular models of KMT2A-AFF1, RS411 and SEM, both derived from childhood leukemias. I use the previously developed BLDA method to identify key-word regions discriminative of the different groupings of samples, and study their epigenomic properties. This amounts to an investigation into the accessible regulatory regions and pathways active in in this uniquely difficult to treat form of leukemia. 
